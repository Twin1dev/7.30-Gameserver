#pragma once

// Dumped with Dumper-7!


#include "../SDK.hpp"

namespace SDK
{
//---------------------------------------------------------------------------------------------------------------------
// FUNCTIONS
//---------------------------------------------------------------------------------------------------------------------


// AnimBlueprintGeneratedClass SK_M249_Skeleton_Dynamic_AnimBP.SK_M249_Skeleton_Dynamic_AnimBP_C
// (None)

class UClass* USK_M249_Skeleton_Dynamic_AnimBP_C::StaticClass()
{
	static class UClass* Clss = nullptr;

	if (!Clss)
		Clss = UObject::FindClassFast("SK_M249_Skeleton_Dynamic_AnimBP_C");

	return Clss;
}


// SK_M249_Skeleton_Dynamic_AnimBP_C SK_M249_Skeleton_Dynamic_AnimBP.Default__SK_M249_Skeleton_Dynamic_AnimBP_C
// (Public, ClassDefaultObject, ArchetypeObject, WasLoaded, LoadCompleted)

class USK_M249_Skeleton_Dynamic_AnimBP_C* USK_M249_Skeleton_Dynamic_AnimBP_C::GetDefaultObj()
{
	static class USK_M249_Skeleton_Dynamic_AnimBP_C* Default = nullptr;

	if (!Default)
		Default = static_cast<USK_M249_Skeleton_Dynamic_AnimBP_C*>(USK_M249_Skeleton_Dynamic_AnimBP_C::StaticClass()->DefaultObject);

	return Default;
}


// Function SK_M249_Skeleton_Dynamic_AnimBP.SK_M249_Skeleton_Dynamic_AnimBP_C.ExecuteUbergraph_SK_M249_Skeleton_Dynamic_AnimBP
// (Final, UbergraphFunction)
// Parameters:
// int32                              EntryPoint                                                       (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void USK_M249_Skeleton_Dynamic_AnimBP_C::ExecuteUbergraph_SK_M249_Skeleton_Dynamic_AnimBP(int32 EntryPoint)
{
	static class UFunction* Func = nullptr;

	if (!Func)
		Func = Class->GetFunction("SK_M249_Skeleton_Dynamic_AnimBP_C", "ExecuteUbergraph_SK_M249_Skeleton_Dynamic_AnimBP");

	Params::USK_M249_Skeleton_Dynamic_AnimBP_C_ExecuteUbergraph_SK_M249_Skeleton_Dynamic_AnimBP_Params Parms{};

	Parms.EntryPoint = EntryPoint;

	UObject::ProcessEvent(Func, &Parms);

}

}


